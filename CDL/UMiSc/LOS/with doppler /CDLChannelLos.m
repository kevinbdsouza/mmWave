...
function [pathGains,sampleTimes,AoDSpec,AoASpec] = CDLChannelLos(model,info,pdp,insize,coupling,Phi)
    ...
    % step 7)
    [HLOS,AoDSpecLOS,AoASpecLOS] = generateClusterGains(model,insize,pdp,coupling,Phi,info.ClusterTypes,'LOS',fc,bw,nSub,interCarSpace);                    % size T_H-by-LOS-by-P-by-R
    [HNLOS,AoDSpecNLOS,AoASpecNLOS] = generateClusterGains(model,insize,pdp,coupling,Phi,info.ClusterTypes,'NLOS',fc,bw,nSub,interCarSpace);  % size T_H-by-(Lin-LOS-NSC)-by-P-by-R
    
            
    
    pathGains = cat(2,HLOS,HNLOS); % size T_H-by-L-by-P-by-R
    AoDSpec = cat(2,AoDSpecLOS,AoDSpecNLOS);
    AoASpec = cat(2,AoASpecLOS,AoASpecNLOS);
...
function [H,AoDSpec,AoASpec] = generateClusterGains(model,insize,pdp,coupling,Phi,allClusterTypes,thisClusterType,fc,bw,nSub,interCarSpace)
