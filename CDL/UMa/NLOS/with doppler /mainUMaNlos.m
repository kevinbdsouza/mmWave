...
for i = 1:length(d2D)
[UMaNlosChan,UMaNlosChanH,txWaveform,Param] = UMaNlos(d2D(i),frequency(fc));
[PDP,AoDSpec,AoASpec] = UMaNlosChan(txWaveform);
[HperUser,~,~] = UMaNlosChanH(txWaveform);
...
pdpUMaNlosF = cat(4,pdpUMaNlosF,pdpPerF);
AoDUMaNlosF = cat(5,AoDUMaNlosF,AoDPerF);
AoAUMaNlosF = cat(5,AoAUMaNlosF,AoAPerF);
