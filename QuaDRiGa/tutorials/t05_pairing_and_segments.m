...
% The link to BS2 is in "WINNER_Indoor_A1_LOS". MT1 has no segments. The rows in "track.scenario"
...
l.no_rx = 2;                                            % Two MTs
l.track(1,1) = qd_track('linear', 0.2 );                % Linear track with 20 cm length
l.track(1,1).name = 'Rx1';                              % Set the MT1 name
l.track(1,1).scenario = {'WINNER_UMa_C2_NLOS';'WINNER_Indoor_A1_LOS'};  % Two Scenarios

l.track(1,2) = qd_track('linear', 0.2 );                % Linear track with 20 cm length
l.track(1,2).name = 'Rx2';                              % Set the MT2 name
...
l.track(1,2).segment_index = [1 3];                     % Set segments
l.track(1,2).scenario = {'WINNER_UMa_C2_LOS','WINNER_UMa_C2_NLOS'};

%%
% We calculate the channel coefficients and plot the list of created segments.

cb = l.init_builder;                                    % Initialize builder
gen_ssf_parameters( cb );                               % Generate small-scale-fading
