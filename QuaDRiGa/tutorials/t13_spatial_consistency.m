...
l.track = qd_track( 'linear' , 50, pi/2 );              % 50 m long track going north
l.track.initial_position = [20 ; 30 ; 1.5 ];            % Set start position and MT height
l.track.interpolate_positions(10);                      % One channel sample every 10 cm
l.track.scenario = '3GPP_38.901_UMi_NLOS';              % Set propagation scenario
...
l.track.no_segments = l.track.no_snapshots;     % Use spatial consisteny for mobility
...
b.scenpar.SC_lambda = 5;                        % Set SSF decorrelation distance to 20 m
...
[len,dist] = l.track.get_length;                % Store the length and distances from start point
...
% smoothly over time. NLOS delays can never be smaller than the LOS delay. In addition, the thck
